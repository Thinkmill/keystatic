...
test('getChildFieldAtPath', () => {
  expect(
    getSchemaAtPropPath(['something', 'blockChild'], falseVal, props)
  ).toBe(blockChild);
  expect(getSchemaAtPropPath(['another', 'value'], falseVal, props)).toBe(
    blockChild2
  );
  expect(getSchemaAtPropPath(['another', 'value'], trueVal, props)).toBe(
    inlineChild
  );
  expect(getSchemaAtPropPath(['inlineChild2'], falseVal, props)).toBe(
    inlineChild2
  );
});
...
test('getAncestorFields', () => {
  ...
  expect(
    getAncestorSchemas(root, ['something', 'blockChild'], falseVal)
  ).toEqual([root, props.something]);
  expect(getAncestorSchemas(root, ['another', 'value'], falseVal)).toEqual([
    root,
    props.another,
  ]);
  expect(getAncestorSchemas(root, ['another', 'value'], trueVal)).toEqual([
    root,
    props.another,
  ]);
