...
export function Heading1({
  ...
  plainLine1: ReactNode;
  plainLine2: ReactNode;
  hilight: ReactNode;
}) {
  const [plainLine1Text, plainLine2Text, hilightText] = [
    plainLine1,
    plainLine2,
    hilight,
  ].map(getTextNode);

  const assistiveText = `${plainLine1Text} ${plainLine2Text} ${hilightText}`;
  ...
  return (
    ...
    >
      ...
      <span className="relative">
        ...
        <span className="relative">{hilightText}</span>
      </span>
    </h1>
  );
}
